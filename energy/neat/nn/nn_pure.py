...
class Neuron(object):
    ...
    def activate(self, manipulation = 0):
        "Activates the neuron with a manipulation addition"
        assert self._type is not 'INPUT'
        return sigmoid(self._update_activation() + self._bias + manipulation, self._response, self._activation_type)
...
class Network(object):
    ...
    #TODO: create parasite network that is a data member of this class, this class then just has to run the parasite and check if it is interfering. We'll need to make sure we reproduce the parasite instead of the host
    #TODO: parasite inputs: current synaptic charge of the nodes it targeted last time? Host's current position? No information at all?
    #TODO: parasite outputs: first number is a node it is targeting, second number is the weight is it giving it
    ...
    def sactivate_parasite(self, parasite_manip, inputs=[]):
        '''Serial (asynchronous) network activation method but with a parasite manipulating the host
        ...
        net_output = []
        for n in self.__neurons[self._num_inputs:]:
            if n._id in parasite_manip:
              n._output = n.activate(parasite_manip[n._id])
            else:
              n._output = n.activate()
            if(n._type == 'OUTPUT'):
                net_output.append(n._output)
